...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      />
      <nav
        className={`sm:dark:border-border fixed bottom-0 left-0 top-0 z-[1] hidden min-h-full w-[70px] flex-col bg-[#09090b] px-3 py-2 transition-all sm:flex sm:w-[64px] sm:dark:border-r ${cn(
          { '!block': show }
        )}`}
      >
          ...
          <div className='flex h-full flex-col items-center justify-between'>
            <div className='flex flex-col items-center'>
              {/* <Link
                onClick={() => setShow(false)}
                href='/'
                className='mt-[3px] rounded-lg p-1 transition-all focus:outline-none active:scale-95'
              >
                <Image
                  className='block'
                  src={SvgWhiteLogo}
